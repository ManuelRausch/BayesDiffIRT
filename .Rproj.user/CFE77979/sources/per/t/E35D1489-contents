...
prior <- function(dist, class, coef = NULL) {
  if (!class %in% c("omega_theta",
    ...
    stop(
      paste0(
        class,
        " not recognized. class should be one of: ",
        "omega_theta, omega_gamma, nu, a, tnd, s_delta, s_beta"
      ),
      call. = FALSE
    )
  }
  ...
  if (!is.null(coef)) {
    stop("Setting individual priors for subjects and items is not yet supported",
         call. = FALSE)
  }

  newPrior(substitute(dist), class)
}

newPrior <- function(dist, class) {
  structure(
    list(
      dist = dist,
      class = class
...
defaultPriors <- function(model = "d") {
  if (model == "d") {
    return(list(
      newPrior(quote(normal(0, 2.5)), "omega_theta"),
      newPrior(quote(normal(0, 0.5)), "omega_gamma"),
  ...
  if (model == "q") {
    return(list(
      newPrior(quote(normal(0, 0.75)), "omega_theta"),
      newPrior(quote(normal(0, 0.5)), "omega_gamma"),
