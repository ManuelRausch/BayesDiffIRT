// To-do q-diffusion model
// with non-centralized parameters
// lower bounds at 1e-6!
...
parameters {
  ...
  vector[nPerson] z_theta; // for sammpling
  vector<lower=0.01>[nItem] nu;          // item drift effects
...
transformed parameters{
  ...
  vector<lower=1e-6>[nPerson] theta;
...
model {
  ...
  for (n in 1:nObs) {
    ...
    real delta = theta[p] / nu[i];  // that's the specific thing of the q diffusion model!
