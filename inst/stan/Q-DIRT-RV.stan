// Q-Diffusion model with random variation in starting point and drift
// Kang et al. (2022) Psychometrica
...
data {
  ...
  real<lower=1e-8> s_delta_prior_par2;

  int<lower=1, upper=2> s_beta_prior_family;
  real s_beta_prior_par1;
  real<lower=1e-8> s_beta_prior_par2;

}

parameters {
  // Drift parameterization, also on the log scale
  vector[nPerson] z_theta; // for sampling
  vector<lower=0.001>[nItem] nu;          // item drift effects
  real<lower=1e-8> omega_theta; // standard deviation of person drift effects (on the log scale)
  real<lower=1e-8> s_delta; // standard deviation of the Gaussian drift rate variability, constant across subjects and items

  // Boundary separation parameterization on log scale
  vector[nPerson] z_gamma; // sample
  vector<lower=0.001>[nItem] a;     // item boundary effects
  real<lower=1e-8> omega_gamma; // standard deviation of person boundary effects (on the log scale)
  real<lower = .001, upper = .999> s_beta; // range of the unifirm starting value variability

  // Nondecision time person parameter
  vector<lower=1e-8, upper=tauUpper>[nPerson] tnd;
}

transformed parameters{
  vector[nPerson] log_theta;
  vector[nPerson] theta;
  log_theta = omega_theta * z_theta; //  person drift
  theta = exp(log_theta);
  vector[nPerson] log_gamma;
  vector[nPerson] gamma;
...
model {
  ...
  for (n in 1:nObs) {
    ...
    real delta_eff = resp_sign(resp[n]) * exp(log_theta[p] - log(nu[i]));
